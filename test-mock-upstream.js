 ...
 * - records the request body it received to the OS temp dir (so injection is observable)
...
const server = http.createServer(async (req, res) => {
  ...
  let parsed = {};
  try { parsed = JSON.parse(body.toString('utf8')); } catch { /* empty body -> {} */ }
  ...
  // Always stream. The injected fields are observed through LAST_REQ_FILE above,
  // never through the stream body.
  sse(res);
});
